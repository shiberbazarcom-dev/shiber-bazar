    ...
    <div className="fixed inset-0 bg-black/70 z-[60] flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl max-w-xl w-full overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-3 bg-gray-50 flex items-center justify-center min-h-[280px]">
          {loading && <div className="w-7 h-7 border-2 border-purple-200 border-t-purple-600 rounded-full animate-spin" />}
        ...
        {url && (
          <div className="px-4 py-2 border-t text-right">
            <a href={url} target="_blank" rel="noreferrer" className="text-xs text-purple-600 hover:underline">নতুন ট্যাবে খুলুন ↗</a>
          </div>
        )}
      </div>
    </div>
          ...
          return (
            <div key={doc.id} className="flex items-center gap-3 p-2.5 bg-gray-50 rounded-xl border border-gray-100">
              <div className="w-9 h-9 rounded-lg bg-white border border-gray-200 flex items-center justify-center flex-shrink-0">
                {isPdf ? <FileText className="h-4 w-4 text-red-400" /> : <ShieldCheck className="h-4 w-4 text-purple-400" />}
              </div>
