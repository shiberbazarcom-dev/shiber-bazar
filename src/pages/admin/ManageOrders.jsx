...
export default function ManageOrders() {
  ...
  const [rejectConfirm, setRejectConfirm] = useState(null) // { id }
  ...
  return (
    <div>
          ...
          {filtered.map(order => (
            <div key={order.id} className={`bg-white rounded-2xl border overflow-hidden transition-all ${selectedIds.has(order.id) ? 'border-blue-300 ring-1 ring-blue-200' : 'border-gray-100'}`}>
              ...
              <div className="px-5 py-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                {/* Product */}
                <div className="space-y-1.5">
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">অর্ডার</p>
      ...
      <ConfirmModal
        ...
        message="এই অর্ডারটি বাতিল হয়ে যাবে।"
        confirmLabel="হ্যাঁ, বাতিল করুন"
        confirmVariant="danger"
        onConfirm={async () => {
          await handleStatus(rejectConfirm.id, 'rejected')
